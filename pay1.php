...
if(isset($_POST['btn_submit'])){    
    ...
    $hey= $_SESSION['name']; 
    $sql1=mysqli_query($con,"SELECT * FROM `workshop` where md5(workshop_name)='".$_GET['name']."' AND md5(workshop_price)='".$_GET['wage']."'"); 
    $row1=mysqli_fetch_assoc($sql1);
    $ep=$row1['workshop_price'];
    $en=$row1['workshop_name'];
    $id1=$row1['workshop_id'];
    ...
    if($rs){
        ...
        header( "location:workvideo.php?videoid=$id1" );
    }
            ...
            <div class="col-12 col-lg-4"></div>
                ...
                <div class="col-12 col-lg-4">
                    <div class="single-ticket-pricing-table style-2 text-center mb-100 wow fadeInUp" data-wow-delay="300ms">
                        ...
                        <? 
                            $sql=mysqli_query($con,"SELECT * FROM `workshop` where md5(workshop_name)='".$_GET['name']."' AND md5(workshop_price)='".$_GET['wage']."'"); 
                            $row=mysqli_fetch_assoc($sql);
                        ?> 
                        <h2 class="ticket-price"><span>₹</span><?=$row['workshop_price']?></h2> 
                        <!-- Ticket Pricing Table Details -->
                        <div class="ticket-pricing-table-details">
                            <p><i class="zmdi zmdi-check"></i> <?=$row['workshop_name']?></p> 
                        </div>
